...
char cl_title[ MAX_CVAR_VALUE_STRING ] = PRODUCT_NAME;
...
{
	Cbuf_ExecuteText(EXEC_NOW, "exec " DEFAULT_FILE "\n");
	Cbuf_Execute(); // Always execute after exec to prevent text buffer overflowing

	// skip the config execution if "safe" is on the command line
	if(!Com_SafeMode())
	{
		char *s;
		s = (char *)Cvar_VariableString( "fs_game" );
		if ( !s[0] ) s = BASEGAME;
		Cbuf_ExecuteText( EXEC_NOW, "exec " CONFIG_PATH_FULL "mod-default\n" );
		Cbuf_Execute();
		Cbuf_ExecuteText( EXEC_NOW, va( "exec " CONFIG_PATH_FULL "mod-%s\n", s ) );
		Cbuf_Execute();

		Cbuf_ExecuteText(EXEC_NOW, "exec " CONFIG_PATH_FULL CONFIG_FILE "\n");
		Cbuf_Execute();
		Cbuf_ExecuteText(EXEC_NOW, "exec " CONFIG_PATH_FULL AUTOEXEC_FILE "\n");
		Cbuf_Execute();
	}
}
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
	...
	FS_Printf( f, Q_NEWLINE "// Generated by " PRODUCT_NAME PRODUCT_VERSION ", do not modify." Q_NEWLINE );
...
//	Swap_Init ();
	...
	Cvar_Get( "sv_master1", MASTER_SERVER_NAME, CVAR_INIT, NULL, NULL, CV_NONE );
	Cvar_Get( "sv_master2", "master.ioquake3.org", CVAR_INIT, NULL, NULL, CV_NONE );
	Cvar_Get( "sv_master3", "master.maverickservers.com", CVAR_INIT, NULL, NULL, CV_NONE );
...
static void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf( f, "// Generated by " PRODUCT_VERSION ", do not modify." Q_NEWLINE );
#ifndef DEDICATED
	Key_WriteBindings( f );
#endif
...
void Com_WriteConfiguration( void ) {
	...
	Com_WriteConfigToFile( CONFIG_PATH_FULL CONFIG_FILE );
...
==================
ReplaceStringTokens
Finds a substring within $(x), searches for cvar name matching x, replaces 
==================
*/
