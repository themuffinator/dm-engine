...
void RE_ScaleCorrection( float *x, float *y, float *w, float *h, const int forceMode ) {
	...
	if ( !mode || backEnd.isHyperspace || ( backEnd.refdef.rdflags & RDF_HYPERSPACE ) ) return;
		...
		case 2:	// screen adjusted uniform scale + virtual screen space
		{
			...
			//ri.Printf( PRINT_ALL, S_COLOR_GREEN "in: vWidth=%i vHeight=%i vxScale=%f vyScale=%f xScale=%f yScale=%f\n", vWidth, vHeight, vxScale, vyScale, xScale, yScale );
			...
			// revert to 640x480
			if ( mx ) mx /= ( glConfig.vidWidth / 640.0f );// *vxScale;
			if ( mw ) mw /= ( glConfig.vidWidth / 640.0f );// * vxScale;
			if ( my ) my /= ( glConfig.vidHeight / 480.0f );// * vyScale;
			if ( mh ) mh /= ( glConfig.vidHeight / 480.0f );// * vyScale;
			...
			//ri.Printf( PRINT_ALL, S_COLOR_RED "out: vWidth=%i vHeight=%i vxScale=%f vyScale=%f xScale=%f yScale=%f\n", vWidth, vHeight, vxScale, vyScale, xScale, yScale );
